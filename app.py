...
def admit():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO Patient(pid, dateofBirth, name, password) VALUES(%s, %s, %s, %s)", (pid, dateofBirth, name, password))
        mysql.connection.commit()
        cur.close()
        #return 'success'
        flash("Patient Added")
...
def createVisit():
    if request.method == 'POST':
        ...
        disDate = '2999-12-30'
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO visit(visitID, pid, illness, adDate, disDate) VALUES(%s, %s, %s, %s,%s)", (visitID, patientID, illness, admitDate, disDate))
        mysql.connection.commit()
        cur.close()
        #return 'success'
        flash("New Visit")
